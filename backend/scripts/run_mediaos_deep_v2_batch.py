#!/usr/bin/env python3
"""Run 15 trials across mediaos-deep-v2 with 3 OpenAI models."""
...
SUITE = ROOT / "examples" / "mediaos-deep-v2"
OUT = ROOT / "backend" / "runs" / "mediaos-deep-v2-batch-report.json"
...
def main() -> int:
    tasks = task_dirs()
    if len(tasks) != 3:
        print(f"expected 3 tasks, found {len(tasks)}", file=sys.stderr)
        return 1
